#' Makes connection between child and parent pages using a link
#' and grabs page numbers of SDTM variables from aCRF file
#' 
#' @param dsin preprocessed input dataset
#' @param dset_defOrigin modified Define Origin file
#' 
#' @example linkPages <- get_linkPages(dsin = crf_preprocess, dset_defOrigin = defineOrigin_variableTab3)
#' 

# get_linkPages <- function(dsin = NULL, debugme = NULL) {
get_linkPages <- function(dsin = NULL, dset_defOrigin = NULL) {
  ...
  for (k in nbrPgs_unq) {
    ...
    if (any(str_detect(aPage, "(SEE CRF PAGE)\\s+\\d+|(SEE CRF PAGES)\\s+\\d+\\s+(AND)\\s+\\d+"))) {
    # if (any(str_detect(aPage, "(SEE CRF PAGE|PAGES)\\s+\\d+"))) {
      # isParent_exist <- str_extract(aPage, "(SEE CRF PAGE)\\s+\\d+")
      isParent_exist <- str_extract(aPage, "(SEE CRF PAGE)\\s+\\d+|(SEE CRF PAGES)\\s+\\d+\\s+(AND)\\s+\\d+")
      ...
      for (pgNbr in pgNbr_parent) {
        ...
        if (any(str_detect(aPage_parent, "(SEE CRF PAGE)\\s+\\d+|(SEE CRF PAGES)\\s+\\d+\\s+(AND)\\s+\\d+"))) {
          
          isParent_exist2 <- str_extract(aPage_parent, "(SEE CRF PAGE)\\s+\\d+|(SEE CRF PAGES)\\s+\\d+\\s+(AND)\\s+\\d+")
          isParent_exist2_fltr <- as.character(isParent_exist2[!is.na(isParent_exist2)])
          pgNbr_parent2 <- as.character(unlist(str_extract_all(isParent_exist2_fltr, "\\d+")))
        ...
        getVars_parent_tmp <- unlist(str_extract_all(aPage_parent, "(\\.\\w{3,})"))
        getVars_parent_tmp <- str_trim(str_replace(getVars_parent_tmp, ".", ""))
        getVars_parent_tmp <- getVars_parent_tmp[!is.na(getVars_parent_tmp)]
        getVars_parent_tmp <- str_extract(getVars_parent_tmp, "\\D+")
        ...
        for (domain in domain_DefOrigin_VarTab_filtered) {
          ...
          if (any(str_detect(getVars_parent, query))) {
        ...
        for (addVars in lst_addVars) {
          ...
          # print(addVars2)
          if (any(str_detect(aPage_parent, addVars2))) { 
            # addVars <- unlist(str_extract_all(aPage_parent, addVars2))
            addVars3 <- as.character(unlist(str_extract_all(aPage_parent, addVars2)))
