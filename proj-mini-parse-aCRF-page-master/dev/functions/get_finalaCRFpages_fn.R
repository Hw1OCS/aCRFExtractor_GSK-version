get_finalaCRFpages <- function (dsin_crf = NULL, dsin_linkPages = NULL, 
  ...
  crf_out_final <- data.frame(page_nbr = character(), sdtm_vars = character(), 
                              domain = character(), Variable = character())
  ...
  for (sel_domain in domain_list) {
    ...
    for (var in vars_list) {
      ...
      if ( any(stringr::str_detect(unique(dsin_linkPages$domain), sel_domain)) ) {
        ...
        dsin_linkPages_sel <- dsin_linkPages_sel[!duplicated(dsin_linkPages_sel[, c("page_nbr", "sdtm_vars")]), ]
