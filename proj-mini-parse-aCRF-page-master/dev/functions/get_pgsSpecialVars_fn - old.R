get_pgsSpecialVars <- function (dsin_unique = NULL, dsin_crf = NULL, dsin_defOrigFinal = NULL, 
  ...
  # if ( stringr::str_detect(variable, "STUDYID$") ) { 
  if ( stringr::str_detect(variable, "STUDYID") ) {          ## 23-Sep-2017
  ...
  # else if ( stringr::str_detect(variable, "VISITNUM$") ) {
  else if ( stringr::str_detect(variable, "VISITNUM") ) {
  ...
  # for (domain in domain_list) {      ## [Haile2Comment, 15-jun-2017]. The program handles per domain only
    ...
    if ( !identical(pgs_domain, character(0)) ) {
      for (i in pgs_domain) {
        ...
        if ( any(stringr::str_detect(str_trim(crf_pg), query_specialVars)) ) {
          # print(domain)
    ...
    # }
      ...
      pgs_specialVars <- sqldf::sqldf(query_pgNbr)
      pgs_specialVars$i_concat <- unlist(stringr::str_replace_all(pgs_specialVars$i_concat, ",", " "))
      ...
      pgs_defOrig_tmp$page_nbr_concat <- unlist(stringr::str_replace_all(pgs_defOrig_tmp$page_nbr_concat, ", ", " "))
    ...
    query_pgNbr2 <- paste(paste("SELECT A.*, group_concat(i)", " AS i_concat ", sep = ""),
                                "FROM pgs_specialVars AS A", sep = "")
    pgs_specialVars <- sqldf::sqldf(query_pgNbr2)
